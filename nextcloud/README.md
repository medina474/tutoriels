...
Pour éviter la cohabitation de systèmes concurents, nous allons installer nexcloud avec Nginx et Postgresql
...
``` sql
create user nextcloud with password 'motdepasse';
create database nextcloud owner nextcloud;

create user onlyoffice with password 'motdepasse';
...
```shell-session
apt install --no-install-recommends rabbitmq-server
```
...
mv /tmp/onlyoffice.gpg /usr/share/keyrings
```


#### Configuration

fichier /var/www/nextcloud/config.php
...
fichier /etc/onlyoffice/documentserver/local.json
